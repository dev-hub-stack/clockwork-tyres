@php
    $record = $this->record;
    $histories = $record->histories()->latest()->take(10)->get();
@endphp
    ...
    @forelse($histories as $history)
        <div class="flex gap-4">
            ...
            <div class="flex flex-col items-center flex-shrink-0">
                ...
                @if(!$loop->last)
                    <div class="w-0.5 flex-1 min-h-8 bg-gray-200 dark:bg-gray-700 mt-2"></div>
                @endif
            </div>
            ...
            <div class="flex-1 pb-4">
                <div class="flex items-start justify-between">
                    <div class="flex-1">
                        ...
                        @if($history->metadata)
                            <div class="mb-2 p-3 bg-gray-50 dark:bg-gray-800 rounded-lg text-xs">
                                ...
                                @if(isset($history->metadata['notes']))
                                    <p class="mt-1 text-gray-700 dark:text-gray-300">{{ $history->metadata['notes'] }}</p>
                                @endif
                            </div>
                        @endif
